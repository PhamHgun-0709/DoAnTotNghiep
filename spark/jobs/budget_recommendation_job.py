...
def minmax_normalize(col_name: str, min_col: str, max_col: str):
    return F.when(F.col(max_col) > F.col(min_col), (F.col(col_name) - F.col(min_col)) / (F.col(max_col) - F.col(min_col))).otherwise(F.lit(0.5))
...
def main() -> None:
    ...
    scored_df = (
        ...
        .withColumn(
            "age_segment",
            F.when(F.col("age").rlike("^[0-9]{2}-[0-9]{2}$"), F.col("age")).otherwise(F.lit("unknown")),
        )
        .withColumn(
            "gender_segment",
            F.when(F.upper(F.col("gender")).isin("M", "F"), F.upper(F.col("gender"))).otherwise(F.lit("unknown")),
        )
        .where((F.col("age_segment") != F.lit("unknown")) & (F.col("gender_segment") != F.lit("unknown")))
    )

    aggregated_df = (
        scored_df.groupBy("campaign_id", "age_segment", "gender_segment")
        .agg(
            F.count("*").alias("ads_count"),
            F.sum("spent").alias("total_spent"),
            F.sum("approved_conversion").alias("total_approved_conversion"),
        ...
        .withColumn(
            ...
            F.when(F.col("total_spent") > 0, F.col("total_approved_conversion") / F.col("total_spent")).otherwise(F.lit(0.0)),
        )
    ...
    scored_segment_df = (
        ...
        .withColumn("min_conversion_per_spent", F.min("conversion_per_spent").over(stat_window))
        .withColumn("max_conversion_per_spent", F.max("conversion_per_spent").over(stat_window))
        ...
        .withColumn(
            "norm_conversion_per_spent",
            minmax_normalize("conversion_per_spent", "min_conversion_per_spent", "max_conversion_per_spent"),
        )
        ...
        .withColumn(
            ...
            F.round(
                ...
                + F.col("norm_conversion_per_spent") * F.lit(0.25)
    ...
    final_df = (
        ...
        .withColumn("segment_id", F.concat_ws("|", F.col("campaign_id"), F.col("age_segment"), F.col("gender_segment")))
        .select(
            "segment_id",
            "campaign_id",
            F.col("age_segment").alias("age"),
            F.col("gender_segment").alias("gender"),
            "ads_count",
            F.round("total_spent", 4).alias("total_spent"),
            F.round("total_approved_conversion", 4).alias("total_approved_conversion"),
            ...
            F.round("conversion_per_spent", 6).alias("conversion_per_spent"),
            "recommendation_score",
